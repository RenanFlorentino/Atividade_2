class HashTable:
	...
	def remove(self, key):
		...
		if self.level1[index1] is not None:
			...
			# Procura e remove a chave no segundo nível
			for entry in self.level1[index1][index2]:
				if entry[0] == key:
					self.level1[index1][index2].remove(entry)
					return
...
table.remove("chave1")
print(table.get("chave1"))  # Saída: None (chave removida)
